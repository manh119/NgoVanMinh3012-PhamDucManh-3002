...
/* Solution Code */
/* decrese tick_blocked and unblock thread t if ticks_blocked time out */
void
...
/*Tính toán lại mức độ ưu tiên cho tất cả thread, và cập nhật ready queues tương ứng*/
void update_priorities (void)
{
  ...
  struct thread *t;
  struct list_elem *e; // ~ thread for each all list
...
/*Cập nhật recent_cpu cho tất cả thread (running, ready, blocked)*/
void update_recent_cpus (void)
{
  ...
  struct thread *t;
  struct list_elem *e; // func for each thread of all list
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
  {
    t = list_entry(e, struct thread, allelem);
    if (t != idle_thread)
...
/*cập nhật biến toàn cục load_avg*/
...
/* Update thread's priority cho thread t */
void
